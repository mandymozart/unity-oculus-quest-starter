...
// Wilberforce Final Camera Effects Pro Unity Plugin 1.2.1
...
namespace Wilberforce.FinalCameraEffectsPro
{
    ...
    public class FinalCameraEffectsProCommandBuffer : MonoBehaviour
    {
        ...
        public string LutFolder = null;
        public int selectedLutIndex = 0;
        ...
        public enum ColorGradingOrderType
        {
            ColorGradingBeforeBloom = 1,
            ColorGradingAfterBloom = 2,
            ColorGradingAtBeginning = 3,
            ColorGradingAtEnd = 4
        }
...
#region Labels
        ...
        private readonly GUIContent colorCorrectionIntensityLabelContent = new GUIContent("Intensity: ", "Blends between color corrected and original image.");
...
#region Effect Implementation Utilities
        ...
        private string[] lutFilesList = null;
        private List<string> lutFilesAssetPathList = null;
        ...
        override public void OnInspectorGUI()
        {
            ...
            FinalCameraEffectsPro.ColorGradingOrderType colorGradingOrder = FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingAtEnd;

            if (script.ColorGradingOrder == 0) colorGradingOrder = FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingAtBeginning;
            else if (script.ColorGradingOrder == 5) colorGradingOrder = FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingAtEnd;
            else if (script.ColorGradingOrder < script.BloomOrder) colorGradingOrder = FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingBeforeBloom;
            else if (script.ColorGradingOrder > script.BloomOrder) colorGradingOrder = FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingAfterBloom;

            var newBloomDofOrder = bloomDofOrder;
            var newVignetteOrder = vignetteOrder;
            var newDistortionOrder = distortionOrder;
            var newColorGradingOrder = colorGradingOrder;
            ...
            if (script.orderFoldout)
            {
                ...
                newColorGradingOrder = (FinalCameraEffectsPro.ColorGradingOrderType)EditorGUILayout.EnumPopup("Color Grading Order: ", colorGradingOrder);
                EditorGUI.indentLevel--;
            }
            ...
            if (newColorGradingOrder != colorGradingOrder || newBloomDofOrder != bloomDofOrder || newVignetteOrder != vignetteOrder || newDistortionOrder != distortionOrder)
            {

                List<FinalCameraEffectsPro.LensEffectType> order = new List<FinalCameraEffectsPro.LensEffectType>();
                ...
                {
                    order.Add(FinalCameraEffectsProCommandBuffer.LensEffectType.Bloom);
                    order.Add(FinalCameraEffectsProCommandBuffer.LensEffectType.DepthOfField);
                    bloomIndex = 0;
                    dofIndex = 1;
                }
                else
                ...
                {
                    order.Insert(bloomIndex, FinalCameraEffectsProCommandBuffer.LensEffectType.Vignette);
                }
                else
                {
                    order.Insert(bloomIndex + 1, FinalCameraEffectsProCommandBuffer.LensEffectType.Vignette);
                }

                for (int i = 0; i < order.Count; i++) if (order[i] == FinalCameraEffectsProCommandBuffer.LensEffectType.DepthOfField) dofIndex = i;

                if (newDistortionOrder == FinalCameraEffectsPro.DistortionOrderType.DistortionBeforeDepthOfField)
                {
                    order.Insert(dofIndex, FinalCameraEffectsProCommandBuffer.LensEffectType.Distortion);
                }
                else
                {
                    order.Insert(dofIndex + 1, FinalCameraEffectsProCommandBuffer.LensEffectType.Distortion);
                }

                order.Insert(0, FinalCameraEffectsProCommandBuffer.LensEffectType.ChromaticAberrations);

                for (int i = 0; i < order.Count; i++) if (order[i] == FinalCameraEffectsProCommandBuffer.LensEffectType.Bloom) bloomIndex = i;

                if (newColorGradingOrder == FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingAtBeginning)
                {
                    order.Insert(0, FinalCameraEffectsPro.LensEffectType.ColorGrading);
                }
                else if (newColorGradingOrder == FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingAtEnd)
                {
                    order.Insert(order.Count, FinalCameraEffectsPro.LensEffectType.ColorGrading);
                }
                else if (newColorGradingOrder == FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingBeforeBloom)
                {
                    order.Insert(bloomIndex, FinalCameraEffectsPro.LensEffectType.ColorGrading);
                }
                else if (newColorGradingOrder == FinalCameraEffectsProCommandBuffer.ColorGradingOrderType.ColorGradingAfterBloom)
                {
                    order.Insert(bloomIndex + 1, FinalCameraEffectsPro.LensEffectType.ColorGrading);
                }

                for (int i = 0; i < order.Count; i++)
                {
                    if (order[i] == FinalCameraEffectsProCommandBuffer.LensEffectType.Bloom) script.BloomOrder = i;
                    if (order[i] == FinalCameraEffectsProCommandBuffer.LensEffectType.ChromaticAberrations) script.ChromaOrder = i;
                    if (order[i] == FinalCameraEffectsProCommandBuffer.LensEffectType.ColorGrading) script.ColorGradingOrder = i;
                    if (order[i] == FinalCameraEffectsProCommandBuffer.LensEffectType.DepthOfField) script.DofOrder = i;
                    if (order[i] == FinalCameraEffectsProCommandBuffer.LensEffectType.Distortion) script.DistortionOrder = i;
                    if (order[i] == FinalCameraEffectsProCommandBuffer.LensEffectType.Vignette) script.VignetteOrder = i;
                }
            }
        ...
        private void ColorCorrectionGUI(FinalCameraEffectsProCommandBuffer script)
        {
            ...
            if (script.colorCorrectionFoldout)
            {
                ...
                if (!String.IsNullOrEmpty(script.LutFolder))
                {
                    if (GUILayout.Button("Refresh LUT List", new GUILayoutOption[] {
                   GUILayout.Width(120.0f),
                }))
                    {
                        buildLUTFileList(script.LutFolder);
                    }
                }

                if (GUILayout.Button("Select LUT Folder", new GUILayoutOption[] {
                   GUILayout.Width(120.0f),
                }))
                {
                    var newFolder = EditorUtility.OpenFolderPanel("Select LUT Folder", Application.dataPath, "");

                    if (!string.IsNullOrEmpty(newFolder))
                    {
                        if (!newFolder.Replace('\\', '/').StartsWith(Application.dataPath.Replace('\\', '/')))
                        {
                            EditorUtility.DisplayDialog("Error while selecting LUT folder", "You must select a subfolder under the 'Assets' directory: " + Application.dataPath, "OK");
                        }
                        else
                        {
                            script.LutFolder = newFolder;
                            if (!String.IsNullOrEmpty(script.LutFolder)) buildLUTFileList(script.LutFolder);
                        }
                    }
                }
                ...
                if (!String.IsNullOrEmpty(script.LutFolder) && lutFilesList == null) buildLUTFileList(script.LutFolder);

                int newSelectedLutIndex = 0;
                if (lutFilesList != null) newSelectedLutIndex = EditorGUILayout.Popup("Select LUT:", script.selectedLutIndex, lutFilesList);

                if (newSelectedLutIndex != script.selectedLutIndex)
                {
                    script.selectedLutIndex = newSelectedLutIndex;
                    var temp = (Texture2D)AssetDatabase.LoadAssetAtPath(lutFilesAssetPathList[script.selectedLutIndex], typeof(Texture2D));
                    if (temp != null) script.ColorCorrectionLutTexture = temp;
                    if (!String.IsNullOrEmpty(script.LutFolder)) buildLUTFileList(script.LutFolder);
                }
        ...
        private void buildLUTFileListWalk(string directory, ref List<string> result, int substrLength)
        {
            var allFiles = System.IO.Directory.GetFiles(directory);
            var allDirectories = System.IO.Directory.GetDirectories(directory);

            foreach (var file in allFiles)
            {
                if (!file.EndsWith(".meta")) result.Add(file);
            }

            foreach (var dir in allDirectories)
            {
                 buildLUTFileListWalk(dir, ref result, substrLength);
            }

        }

        private void buildLUTFileList(string directory)
        {
            lutFilesAssetPathList = new List<string>();
            
            buildLUTFileListWalk(directory, ref lutFilesAssetPathList, directory.Length);

            lutFilesAssetPathList.Sort();
            lutFilesList = new string[lutFilesAssetPathList.Count];

            for (int i = 0; i < lutFilesAssetPathList.Count; i++)
            {
                lutFilesList[i] = lutFilesAssetPathList[i].Substring(directory.Length + 1).Replace('\\', '/');
                lutFilesAssetPathList[i] = "Assets/"+ lutFilesAssetPathList[i].Replace('\\','/').Substring(Application.dataPath.Length + 1);
            }
        }
